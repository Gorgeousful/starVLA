...
#    `Qwen_KI.project_layers` does the LayerNorm+Linear compression of
...
class QwenKIDefaultConfig:
    """QwenKI framework default parameters.
    ...
    action_model: dict = field(
        default_factory=lambda: {
            ...
            "diffusion_model_cfg": {
                ...
                # and Qwen_KI.project_layers compress VL hidden to this dim.
...
class Qwen_KI(baseframework):
    """
    Multimodal vision-language-action model (QwenKI variant).
    ...
    def __init__(
        ...
        **kwargs,
        ...
        ki_cfg = getattr(self.config.framework, "ki", {})
    ...
    def forward(
        ...
        **kwargs,
        ...
        fast_action_loss = qwenvl_outputs.loss
        if fast_action_loss is None or torch.isnan(fast_action_loss):
            fast_action_loss = torch.tensor(0.0, device=self.qwen_vl_interface.model.device)

        vl_embs_list = list(qwenvl_outputs.hidden_states[-self.num_action_dit_layers:])
        vl_embs_for_flow = (
            [vl_emb.detach() for vl_emb in vl_embs_list]
            if self.ki_detach_vlm_for_flow
            else vl_embs_list
        )
        vl_embs_for_flow = self._project_vl_hidden_for_action(vl_embs_for_flow)
        base_hidden = vl_embs_for_flow[-1]
        ...
        total_loss = (
            self.ki_flow_loss_weight * flow_action_loss
            + self.ki_fast_loss_weight * fast_action_loss
        )
    ...
    def build_training_solution(self, example: dict, fast_tokens) -> str:
        ...
        annotation_solution = self.build_annotation_solution(example)
        if not annotation_solution:
            return action_token_text
        return f"{annotation_solution}\n{action_solution}"

    def build_annotation_solution(self, example: dict) -> str:
        parts = []
        subtask = example.get("subtask")
        if isinstance(subtask, dict) and subtask.get("description"):
            parts.append(f"[SUBTASK] {subtask.get('description')}")
    ...
    def _sort_grounding_objects(self, objects: list[dict]) -> list[dict]:
        def sort_key(indexed_obj):
            index, obj = indexed_obj
            object_key = str(obj.get("object_key", ""))
            name = str(obj.get("name", "")).lower()
            is_gripper = object_key == "0::0" or "gripper" in name
            is_manipulated = obj.get("is_manipulated") is True
            if is_gripper:
                group = 0
            elif is_manipulated:
                group = 1
        ...
        return [obj for _, obj in sorted(enumerate(objects), key=sort_key)]
    ...
    def _normalize_bbox_sequence(self, bbox_sequence) -> list[list[int]]:
        custom_annotation_cfg = getattr(self.config.datasets.vla_data, "custom_annotation", {})
        bbox_size = custom_annotation_cfg.get("bbox_size", [256, 256])
        width, height = float(bbox_size[0]), float(bbox_size[1])
        ...
        for bbox in bbox_sequence or []:
            ...
            normalized.append(
                [
                    self._normalize_coord_to_1000(x1, width),
                    self._normalize_coord_to_1000(y1, height),
                    self._normalize_coord_to_1000(x2, width),
                    self._normalize_coord_to_1000(y2, height),
                ]
            )
        return normalized

    def _normalize_coord_to_1000(self, value, size: float) -> int:
        if size <= 0:
            return 0
